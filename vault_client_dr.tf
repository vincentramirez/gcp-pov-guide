// 3 Vault Clients for DR
resource "google_compute_instance" "vault-client-dr" {
  depends_on   = ["google_compute_instance.consul-server-dr"]
  count        = "${var.counts["vault-client"]}"
  name         = "vault-client-dr-${count.index}"
  machine_type = "${var.machineTypes["vault-client"]}"
  zone         = "${var.regionSecondary}-b"
  tags         = ["${var.consulNetworkTag["dc2"]}", "consul-client", "http-server"]
  ...
  provisioner "file" {
    ...
    {
      ${jsonencode("service")}: {
        ${jsonencode("id")}: ${jsonencode("vault-client-dr-${count.index}")},
  ...
  provisioner "file" {
    ...
    content = <<JSON
    {
      ...
      ${jsonencode("node_name")}: ${jsonencode("vault-client-dr-${count.index}")},
      ${jsonencode("datacenter")}: ${jsonencode("dc2")},
