// 5 Consul Nodes
resource "google_compute_instance" "consul-server-dr" {
  count        = "${var.counts["consul-server"]}"
  name         = "consul-server-dr-${count.index}"
  machine_type = "${var.machineTypes["consul-server"]}"
  zone         = "${var.regionSecondary}-b"
  tags         = ["${var.consulNetworkTag["dc2"]}", "consul-server", "http-server"]
  ...
  provisioner "file" {
    ...
    content = <<JSON
    {
      ...
      ${jsonencode("node_name")}: ${jsonencode("consul-server-dr-${count.index}")},
